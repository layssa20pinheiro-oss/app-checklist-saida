...
export default function ChecklistPage() {
  ...
  const salvarETerminar = async () => {
    setLoading(true);
    try {
      // PREENCHENDO TODAS AS GAVETAS POSSÍVEIS PARA EVITAR O ERRO 400
      const dadosParaSalvar = { 
          evento: form.evento || 'Evento sem nome', 
          local: form.local || '', 
          responsavel: form.responsavel || 'Equipe',
          presentes: form.presentes || '',
          convidados: form.convidados || '',
          observacoes: form.observacoes || '',
          obs: form.observacoes || '', // Preenche os dois para garantir
          itens: itens || [], 
          foto_url: fotoUrl || '',
          pdf_url: 'link-pendente', // Se o banco exigir, isso engana ele temporariamente
          evento_id: id || null
      };
      ...
      // ARMADILHA DE ERRO: VAI MOSTRAR NA SUA TELA EXATAMENTE O QUE O BANCO FALOU
      if (res.error) {
    ...
    <div className="min-h-screen bg-[#7e7f7f] p-4 flex flex-col items-center font-sans">
      ...
      {etapa === 'resumo' && (
        <div className="w-full flex flex-col items-center pb-24 animate-in fade-in">
          <div className="w-[380px] bg-[#7e7f7f] p-6 flex flex-col items-center">
