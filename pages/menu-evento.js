      ...
      <div className="w-full max-w-md">
        ...
        <div className="space-y-6">
          <div className="bg-white rounded-[35px] p-2 shadow-xl">
             <Link href={`/checklist?eventoId=${id}`} className="w-full p-6 flex items-center gap-4 text-left">
             ...
             <Link href={`/historico?eventoId=${id}`} className="flex items-center justify-center gap-2 py-4 border-t border-gray-50 text-[#ded0b8]">
                <History size={16} />
                <span className="text-[10px] font-bold uppercase tracking-[2px]">Acessar Histórico</span>
             </Link>
          </div>

          <Link href={`/lista?eventoId=${id}`} className="bg-white p-6 rounded-[35px] shadow-xl flex items-center gap-4 text-left group">
